...
void *kcs_thread(void *unused) {
  ...
  while(1) {
    read_len/*req_len*/ = read(kcs_fd, req_buf, sizeof(req_buf));
    if (read_len/*req_len*/ > 0) {
      req_len = (unsigned char)read_len;
    ...
    /*res_len*/write_len = write(kcs_fd, res_buf, res_len);
    if(write_len < 0)
    {
        nanosleep(&req, &rem);
        continue;
    }    
    res_len = (unsigned char)write_len;
...
main(int argc, char * const argv[]) {
  ...
  char cmd[256], device[256], file[256];
  ...
  sprintf(file, "mknod /dev/ast-kcs.%d c 10 63", kcs_channel_num);
  system(file);
  ...
  if(kcs_fd <= 0)
  {
      syslog(LOG_WARNING, "kcsd: can not open kcs device %s, errno = %d\n", device, errno);
      exit(-1);
  }    
